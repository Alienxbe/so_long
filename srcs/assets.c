...
static void	init_asset_sheet(void *mlx_ptr, t_assets *asset, char *filename, t_point size)
{
	t_img	img;
	t_point	count;
	int		id;
	...
	if (asset->list)
	{
		...
		while (++id < asset->count)
		{
			...
			// ft_printf("New asset from (%d): x1(%d) y1(%d) x2(%d) y2(%d)\n", id,
			// 	0 + size.x * (id % count.x),
			// 			0 + size.y * (id / count.y),
			// 			size.x + size.x * (id % count.x),
			// 			size.y + size.y * (id / count.y));
		}
...
{
	init_asset_sheet(d->mlx_ptr, &d->player, XPM_PLAYER, ft_new_point(64, 64));
	init_asset_sheet(d->mlx_ptr, &d->tiles, XPM_TILESET, ft_new_point(32, 32));
}
